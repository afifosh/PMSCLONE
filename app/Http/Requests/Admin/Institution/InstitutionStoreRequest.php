...
class InstitutionStoreRequest extends FormRequest
{
  ...
  public function rules(): array
  {
    $rules = [
        ...
        'website' => ['nullable', 'string', 'max:255', 'unique:institutions,website'],
    ...
    // Add the institution's ID to the unique rule for update operations
    if ($this->method() == 'PUT' || $this->method() == 'PATCH') {
      $institutionId = $this->institution->id ?? null;
      $rules['name'] = [
          'required',
          'string',
          'max:255',
  ...
  public function messages(): array
  {
    return [
      ...
      'name.string' => 'Institution name must be a string',
      'name.max' => 'Institution name should not be greater than 255 characters',
      'country_id.required' => 'Country is required',
      'avatar.mimetypes' => 'Institution image must be an image file.',
