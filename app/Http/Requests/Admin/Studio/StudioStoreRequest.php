...
class StudioStoreRequest extends FormRequest
{
  ...
  public function rules(): array
  {
    $rules = [
        ...
        'email' => ['nullable', 'email', 'max:255', 'unique:studios,email'],
    ...
    // Add the studio's ID to the unique rule for update operations
    if ($this->method() == 'PUT' || $this->method() == 'PATCH') {
      $studioId = $this->studio->id ?? null;
      $rules['name'] = [
          'required',
          'string',
          'max:255',
  ...
  public function messages(): array
  {
    return [
      ...
      'name.max' => 'Studio name should not be greater than 255 characters',
      'country_id.required' => 'Country is required',
      'avatar.mimetypes' => 'Studio image must be an image file.',
