...
class Manager
{
    ...
    public function createMissing($guard = 'api')
    {
        ...
        Permission::unguarded(function () use ($guard) {
            $repository = resolve(PermissionRepository::class);

            foreach ($this->all() as $permission) {
                $repository->firstOrCreate(['name' => $permission, 'guard_name' => $guard]);
            }
        });
    }
     ...
     * @return void
     */
    protected function registerGroup(Closure $callback)
    {
        $callback($this);
    }
