...
class SharedFilesDataTable extends DataTable
{
  ...
  public function dataTable(QueryBuilder $query): EloquentDataTable
  {
    return (new EloquentDataTable($query))
      ...
      ->addColumn('draft', function($row){
        return $row->file->rfp->name;
      })
      ...
      ->addColumn('action', function ($fileShare) {
        return view('admin.pages.rfp.shared-files.action', compact('fileShare'));
      })
  ...
  {
    $query = $model->mineShared()->latest()->newQuery();
    return $query->applyRequestFilters()->with(['user', 'sharedBy', 'file', 'file.rfp']);
  }
  ...
  {
    return [
      Column::make('draft')->title('RFP Draft Name'),
