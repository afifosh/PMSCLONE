    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('status', function (Form $form) {
                ...
                if ($form->is_active == 1) {
                    $st = '<div class="form-check form-switch">
                            <input class="form-check-input chnageStatus" checked type="checkbox" role="switch" id="' . $form->id . '" data-url="' . route('admin.applications.settings.form.status', $form->id) . '">
                            </div>';
                } else {
                    $st = '<div class="form-check form-switch">
                            <input class="form-check-input chnageStatus" type="checkbox" role="switch" id="' . $form->id . '" data-url="' . route('admin.applications.settings.form.status', $form->id) . '">
                        </div>';
                }
                return $st;
            })
            ->addColumn('action', function (Form $form) {
                $hashids = new Hashids();
                return view('admin.pages.applications.form.action', compact('form', 'hashids'));
            })->editColumn('created_at', function (Form $form) {
                return $form->created_at->format('Y-m-d h:i:s');
                // return UtilityFacades::date_time_format();
            })
    ...
    public function html()
    {
        ...
        if($canCreateForm){
            $buttonsConfig[] =  [
                ...
                'action' => "function ( e, dt, node, config ) { window.location = '" . route('admin.applications.settings.forms.add') . "'; }",
            ];
        }
