...
namespace App\DataTables\Admin\Applications\Form;
...
class FormTemplateDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ->eloquent($query)
            ->addIndexColumn()
            ->editColumn('created_at', function ($FormTemplate) {
                return $FormTemplate->created_at->format('d M Y');
                ...
                if ($FormTemplate->status == 1) {
                    $a = '<div class="form-check form-switch">
                                            <input class="form-check-input chnageStatus" checked type="checkbox" role="switch" id="' . $FormTemplate->id . '" data-url="' . route('admin.applications.settings.formTemplate.status', $FormTemplate->id) . '">
                                        </div>';
                } else {
                    $a =  '<div class="form-check form-switch">
                                            <input class="form-check-input chnageStatus" type="checkbox" role="switch" id="' . $FormTemplate->id . '" data-url="' . route('admin.applications.settings.formTemplate.status', $FormTemplate->id) . '">
                                        </div>';
                }
                return $a;
            })
            ->addColumn('action', function (FormTemplate $FormTemplate) {
                return view('admin.pages.applications.form-template.action', compact('FormTemplate'));
            })
    ...
    public function html(): HtmlBuilder
    {
        ...
        if ($canCreateFormTemplate) {
            $buttonsConfig[] = [
                ...
                'action'    => " function ( e, dt, node, config ) { window.location = '" . route('admin.applications.settings.form-templates.create') . "'; }"
            ];
        }
