...
use Modules\Chat\Events\UpdatesEvent;
use Modules\Chat\Exceptions\ApiOperationFailedException;
use Modules\Chat\Models\ArchivedUser;
use Modules\Chat\Models\BlockedUser;
use Modules\Chat\Models\ChatRequestModel;
use Modules\Chat\Models\Conversation;
use Modules\Chat\Models\Group;
use Modules\Chat\Models\GroupMessageRecipient;
use Modules\Chat\Models\GroupUser;
use Modules\Chat\Models\LastConversation;
...
use Modules\Chat\Models\SocialAccount;
use Modules\Chat\Models\User;
use Modules\Chat\Models\UserStatus;
    ...
    public function getConversation($id, $input)
    {
        ...
        if ($isGroup) {
            ...
            $commonGroups = Group::with(['users' => function ($q) {
                $q->select(['admins.first_name', 'admins.id']);
            }])->whereHas('users', function ($q) use ($id, $authUser) {
                return $q->whereIn('admins.id', [$id, $authUser->id]);
            }, '>=', 2)
                ->get()->toArray();
        ...
        $unreadCount = $countQuery->where('conversations.status', '=', 0)->where('to_id', '=', getLoggedInUserId())->count();
