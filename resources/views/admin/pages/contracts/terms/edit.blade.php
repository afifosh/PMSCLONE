{!! Form::model($contract, ['route' => ['admin.contracts.terms.update', [$contract->id, 0]], 'method' => 'PUT']) !!}

<div class="row">
    {{-- Subject --}}
    <div class="form-group col-6">
        {{ Form::label('subject', __('Subject'), ['class' => 'col-form-label']) }}
        {!! Form::text('subject', null, ['class' => 'form-control', 'required'=> 'true', 'disabled', 'placeholder' => __('Subject')]) !!}
    </div>
    {{-- project --}}
    <div class="form-group col-6">
      ...
      {!! Form::select('project_id', $projects, $contract->project_id, [
        ...
        'data-url' => route('resource-select', ['Project']), 'disabled'
    ...
    <div class="form-group col-6">
      ...
      'class' => 'form-select globalOfSelect2Remote',
      'data-url' => route('resource-select', ['Program']), 'disabled'
      ]) !!}
    </div>
    {{-- customer --}}
    <div class="form-group col-6">
      ...
      'data-url' => route('resource-select-user', ['Company']), 'disabled'
      ]) !!}
    </div>
    {{-- types --}}
    <div class="form-group col-6">
      {{ Form::label('type_id', __('Contract Type'), ['class' => 'col-form-label']) }}
      {!! Form::select('type_id', $types, $contract->type_id, ['class' => 'form-select globalOfSelect2', 'disabled']) !!}
    </div>
    <div class="form-group col-6">
      {{ Form::label('refrence_id', __('Refrence Id'), ['class' => 'col-form-label']) }}
      {!! Form::text('refrence_id', null, ['class' => 'form-control', 'required'=> 'true', 'placeholder' => __('Refrence Id'), 'disabled']) !!}
    </div>
    <div class="form-group col-6">
      ...
      {!! Form::select('currency', $currency ?? [], $contract->currency, [
        ...
        'class' => 'form-select globalOfSelect2Remote',
        'data-url' => route('resource-select', ['Currency']), 'disabled'
        ])!!}
    </div>
    {{-- value --}}
    <div class="form-group col-6">
    ...
    <div class="form-group col-12">
      ...
      {!! Form::textarea('description', null, ['class' => 'form-control', 'rows' => 3, 'placeholder' => __('Description'), 'disabled']) !!}
    </div>
...
<script>
  ...
  function calContEndDate()
  {
    ...
    if(!startDate || !count || !unit) return;
    if (count && unit) {
