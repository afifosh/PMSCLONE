...
<script>
  ...
  @endif
  function populateFolders(account_id, active_folder_id = null) {
  ...
  function populateMessages(account_id, folder_id, page) {
    ...
    $.ajax({
      ...
      type: "GET",
      success: function (response, status) {
        window.active_account_id = account_id;
        window.active_folder_id = folder_id;
  ...
  function initLocSelect2(){
    $('.locSelect2').select2({
      ...
      createTag: function (params) {
        // Validate the tag as an email address
  ...
  function bulkAction(action, id = null) {
    ...
    if (action == 'delete') {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          await $.ajax({
            url: "{{url('admin/core/api/emails/actions/email-account-message-delete/run')}}",
            type: "POST",
            data: {
              account_id: window.active_account_id,
              folder_id: window.active_folder_id,
...
<div class="app-email card">
  <div class="row g-0">
    <!-- Email Sidebar -->
    <div class="col app-email-sidebar border-end flex-grow-0" style="width:auto;overflow-y: auto;"
      ...
      <div class="btn-compost-wrapper d-grid">
        ...
        <button class="btn btn-primary btn-compose" onclick="doAction('compose');" data-bs-toggle="modal"
        ...
        <div class="emails-list-header p-3 py-lg-3 py-2">
          ...
          <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center w-100">
              ...
              <div class="mb-0 mb-lg-2 w-100">
                <div class="input-group input-group-merge shadow-none">
                  ...
                  <input type="text" onkeyup="searchMails(this);" id="mail-search"
                    class="form-control email-search-input border-0" placeholder="Search mail" aria-label="Search mail"
            ...
            <div class="d-flex align-items-center mb-0 mb-md-2">
              ...
              <div class="dropdown">
                <i class="ti ti-dots-vertical cursor-pointer" id="emailsActions" data-bs-toggle="dropdown"
    ...
    <div id="no-folders-area" style="display:none; margin:auto" class="col app-emails-list">
      <div class="text-center mb-3">
        <div class="card-body">
          ...
          <p class="card-text">This account has no active folders. Enable active folders by editing the mail account,
            the active folders will be the folders that will be synchronized to the application.</p>
          @if(auth()->user()->hasPermission(['Owner','Editor','Contributor'],$account))
          <a href="javascript:editAccount();" id="activate-folders" class="btn btn-primary">Activate Folders</a>
          @endif
          <a href="{{url('/admin/mail/accounts/manage-accounts')}}" id="manage-accounts"
            class="btn btn-secondary">Manage Accounts</a>
        </div>
      </div>
    </div>
  ...
  <!-- Compose Email -->
  <div class="app-email-compose modal" data-bs-backdrop="static" id="emailComposeSidebar" tabindex="-1"
          ...
          <form class="email-compose-form" id="email-compose-form" enctype="multipart/form-data">
            ...
            <div class="email-compose-to d-flex justify-content-between align-items-center">
              ...
              <div class="select2-primary border-0 shadow-none flex-grow-1 mx-2">
                <select class="form-control border-0 shadow-none flex-grow-1 mx-2 locSelect2" multiple name="to[]" id="to"></select>
              </div>
            ...
            <div class="email-compose-cc d-none">
              ...
              <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="select2-primary border-0 shadow-none flex-grow-1 mx-2">
                 <select class="form-control border-0 shadow-none flex-grow-1 mx-2 locSelect2" multiple name="cc[]" id="cc"></select>
                </div>
              </div>
            </div>
            <div class="email-compose-bcc d-none">
              ...
              <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="select2-primary border-0 shadow-none flex-grow-1 mx-2">
                 <select class="form-control border-0 shadow-none flex-grow-1 mx-2 locSelect2" multiple name="bcc[]" id="bcc"></select>
                </div>
              </div>
            </div>
            <hr class="container-m-nx my-2">
            <div class="email-compose-subject d-flex align-items-center mb-2">
              ...
              <input type="text" class="form-control border-0 shadow-none flex-grow-1 mx-2" id="subject" name="subject"
            ...
            <div class="email-compose-actions d-flex justify-content-between align-items-center mt-3 mb-3">
              <div class="d-flex align-items-center">
                <div class="">
                  <button type="button" id="send-email" class="btn btn-primary"><i
                      class="ti ti-send ti-xs me-1"></i>Send</button>
                  <button type="button" id="reply-email" onclick="replyEmail(this,$(this).data('id'));"
                    class="btn btn-primary " style="display:none !important"><i
                      class="ti ti-send ti-xs me-1"></i>Reply</button>
                  <button type="button" id="forward-email" onclick="forwardEmail(this,$(this).data('id'));"
                    class="btn btn-primary " style="display:none !important"><i
                      class="ti ti-send ti-xs me-1"></i>Forward</button>
...
<div class="col-md-12 mb-4">
  <div class="row justify-content-center py-5">
    <div class="col-md-8 col-lg-6 mt-4">
      ...
      <p class="mb-3 text-center"> Connect an account to start sending and organize emails in order close deals faster
      </p>
      <div class="row">
        <div class="col-md-6">
          <ul class="p-0 m-0">
            <li class="mb-4 pb-1 d-flex justify-content-between align-items-center">
              <div class="badge bg-label-danger rounded p-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
                  viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" style="width: 20px;"
                  class="w-20 text-primary-600">
            ...
            <li class="mb-4 pb-1 d-flex justify-content-between align-items-center">
              <div class="badge bg-label-danger rounded p-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
                  ...
                  class="h-6 w-6 text-primary-600" style="width: 20px;">
                  <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <li class="mb-4 pb-1 d-flex justify-content-between align-items-center">
              <div class="badge bg-label-danger rounded p-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
                  viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"
                  class=" bg-label-danger" style="width: 20px;">
                  <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="col-md-6">
          <ul class="p-0 m-0">
            ...
            <li class="mb-4 pb-1 d-flex justify-content-between align-items-center">
              <div class="badge bg-label-primary rounded p-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
                  viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"
                  class="h-6 w-6 text-primary-600" style="width: 20px;">
                  <path stroke-linecap="round" stroke-linejoin="round"
      ...
      <div class="d-flex justify-content-center flex-wrap gap-4 mt-2">
        ...
        <button data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddUser"
          onclick="localStorage.setItem('acc_type','personal');" class="btn btn-primary">Connect Personal
          Account</button>
        @endcan
      </div>
    </div>
