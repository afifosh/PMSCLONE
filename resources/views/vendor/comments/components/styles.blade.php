<style>
    {{ $stylesheet }}

    :root {
    --comments-spacing: 0.75rem;  /*default: 2.5*/
  }
    .comments {
    /* --comments-color-background: rgb(34, 34, 34);
    --comments-color-background: rgb(34, 34, 34); */
    /* --comments-color-background-nested: rgb(34, 34, 34);
    --comments-color-background-paper: rgb(55, 51, 51); */
    --comments-color-background-info: rgb(104, 89, 214);

    --comments-color-reaction: rgba(205, 84, 91);
    --comments-color-reaction-hover: rgba(205, 84, 91);
    --comments-color-reacted: rgba(205, 84, 91);
    --comments-color-reacted-hover: rgba(205, 84, 91, 0.6);

    --comments-color-border: rgb(221, 221, 221);

    --comments-color-text:black;
    --comments-color-text-dimmed: rgb(164, 164, 164);
    --comments-color-text-inverse: black;

    --comments-color-accent: rgba(205, 84, 91);
    --comments-color-accent-hover: rgba(205, 84, 91, 0.5);

    --comments-color-danger: rgb(225, 29, 72);
    --comments-color-danger-hover: rgb(225, 29, 72, 0.75);

    /* --comments-color-success: rgb(10, 200, 134);
    --comments-color-success-hover: rgb(10, 200, 134, 0.75); */

    --comments-shadow: 0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1);
  }
</style>
