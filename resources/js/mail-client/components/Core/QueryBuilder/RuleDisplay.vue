<template>
  ...
  >
    {{ $t('core::filters.conditions.' + condition) }}
  </span>
  <div
    v-if="rule.type === 'rule'"
...
const original = computed(() => {
  const originalObject = find(store.state['filters'].rules[props.identifier], [
    'id',
    query.value.rule,
  ])

  if (query.value.operand) {
    return find(originalObject.operands, operand => {
...
function formattedValueLabel(formatter) {
  if (isBetween.value) {
    if (isValueEmpty(query.value.value)) {
      return ''
    }
